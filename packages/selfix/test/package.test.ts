...
it.each([
  { linked: false, standalone: false },
  { linked: true, standalone: false },
  { linked: false, standalone: true },
])("smoke verifies installation paths and cleanup: %j", ({ linked, standalone }) => {
...
`,
  ...
  const originalScript = fileURLToPath(new URL("../scripts/smoke-install.mjs", import.meta.url))
  const script = standalone ? path.join(dir, "smoke-install.mjs") : originalScript
  if (standalone) cpSync(originalScript, script)
  const result = spawnSync(
    process.execPath,
    [script, archive, ...(standalone ? ["3.2.13", "4.0.0"] : [])],
    {
      encoding: "utf8",
