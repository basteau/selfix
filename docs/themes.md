...
Custom classes retain declarations introduced by `@apply` in the CSS entry and imported
stylesheets, including within the supported nested selectors and conditional blocks above:
...
Here `card` has layout, spacing, and raw color effects. A protected component using it
can receive both `no-restyle` and `no-raw-colors`, with each finding located at its class
in the original Vue SFC. Direct and applied effects do not create duplicate rule findings.

Applied utilities use the same loaded Tailwind theme and compiler as ordinary classes.
Semantic theme colors remain semantic even when compiled to literal values; stock palette
references (including inline redefinitions of stock names) and arbitrary literal colors remain raw.
Literal colors authored in CSS `@utility` definitions are checked for both ordinary and applied
utilities. Direct CSS literals are still checked when mixed with semantic applied utilities.
For inspection, single-color `--theme(--color-name)` lookups retain their token identity;
unknown colors and lookups with additional arguments fail explicitly. Functional utility
declarations that combine an authored `transparent` value (including quoted alternatives)
with `--value()` or `--modifier()` also fail explicitly: keep authored transparency in a
separate unconditional declaration. This avoids confusing omitted source values with
transparency generated by Tailwind's opacity handling.

selfix supports whitespace-separated utilities that the loaded compiler can inspect, within
supported style rules. Unknown utilities, unsupported application contexts, and utilities
that cannot be inspected fail theme loading explicitly. Top-level `@apply`, `@apply` inside
`@property` or standalone definition blocks, and legacy standalone `!important` arguments
are not supported. This inspection combines possible effects without evaluating the cascade
or whether a variant or conditional rule currently matches.
