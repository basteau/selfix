...
Public runtime exports are `createLinter`, `lintSource`, `defineConfig`, and `ruleNames` (the six names in the rule table's order). Public type exports are `Config`, `ProjectOptions`, `ComponentDefinition`, `ComponentProps`, `ClassProps`, `Contract`, `Message`, `RuleName`, `RuleOptions`, `RuleSetting`, `Severity`, `Category`, `Diagnostic`, and `LinterOptions`. `defineConfig(config)` validates and returns the config. Internal collectors, validators, and compiler helpers are not package exports.
