...
Escaped identifiers (including `.hover\:card` and hexadecimal CSS escapes) are rejected explicitly; selfix never registers a partial name. Other selector functions, nested negation, and descendant, child, or sibling relationships involving classes are unsupported. Nested selectors support compounds beginning with one parent reference (`&`) per list branch, such as `&:hover`, `&.active:not(.disabled)`, and `&:hover, &::before`. Each branch inherits the enclosing positive class names and adds its own positive classes; negated classes and attribute text remain excluded. Multiple nesting levels and nested `@media`, `@supports`, `@container`, and `@starting-style` blocks preserve ownership and literal declaration values. Implicit descendants (`.card { .child { ... } }`), explicit relationships (`& .child`, `& > span`), parent references inside selector functions, and other nested at-rule blocks are unsupported (`@property` registrations remain ignored). These forms fail theme loading with an `Unable to inspect CSS: unsupported selector` error that includes the selector and reason, even if rules are disabled. Rewrite the selector into supported compounds where equivalent; do not interpret this inspection failure as a clean lint result. Class-free element relationships in reset CSS do not create class associations.
...
- Component recognition uses configured names and import strings. Supported source discovery adds definition paths and verified literal prop choices to restyling findings. There is no cross-file wrapper tracing or autofix. Configure wrapper policies explicitly and choose component props from the component's actual API. Run selfix separately from ESLint or Oxlint.
